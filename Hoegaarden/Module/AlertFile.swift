...
open class SweetAlert: UIViewController {
    ...
    fileprivate func setupTitleLabel() {
        ...
        titleLabel.font = Typography.subtitle2.font
    ...
    fileprivate func setupSubtitleTextView() {
        ...
        subTitleTextView.font = Typography.body2.font
    ...
    fileprivate func resizeAndRelayout() {
        ...
        if imageView != nil {
            imageView.frame = CGRect(x: 26, y: 31, width: 24, height: 24)
        ...
        if self.subTitleTextView.text.isEmpty == false {
            ...
            subTitleTextView.frame = CGRect(x: 24, y: 64, width: 268, height: textViewHeight)
            contentView.addSubview(subTitleTextView)
        ...
        for button in buttons {
            ...
            buttonRect.append(string.boundingRect(with: CGSize(width: width, height: 0.0),
                                                  options: NSStringDrawingOptions.usesLineFragmentOrigin,
                                                  attributes: [NSAttributedString.Key.font: button.titleLabel!.font as Any],
                                                  context: nil)) }
        ...
        for i in 0 ..< buttons.count {
            buttons[i].frame = CGRect(x: buttonX, y: 144, width: 86, height: 41)
            buttonX = buttons[i].frame.origin.x + kWidthMargin + buttonRect[i].size.width + 50.0
            buttons[i].layer.cornerRadius = 4
            self.contentView.addSubview(buttons[i])
            buttons[i].addTarget(self, action: #selector(SweetAlert.pressed(_:)), for: .touchUpInside)
        }
        ...
        if y > kMaxHeight {
            ...
            for button in buttons {
                ...
                button.frame = CGRect(x: bFrame.origin.x, y: bFrame.origin.y - diff, width: 86, height: 41)
            }
            y = kMaxHeight
        }
        
        contentView.frame = CGRect(x: (mainScreenBounds.size.width - kContentWidth) / 2.0, y: (mainScreenBounds.size.height - y) / 2.0, width: 316, height: 209)
    ...
    open func showAlert(_ title: String, subTitle: String?, style: AlertStyle,buttonTitle: String,buttonColor: UIColor,
                        otherButtonTitle: String?, action: ((_ isOtherButton: Bool) -> Void)? = nil) -> SweetAlert {
    ...
    open func showAlert(_ title: String, subTitle: String?, style: AlertStyle, buttonTitle: String, buttonColor: UIColor,
                        otherButtonTitle: String?, otherButtonColor: UIColor?, action: ((_ isOtherButton: Bool) -> Void)? = nil) {
        userAction = action
        ...
        case let .customImage(imageFile):
            if let image = UIImage(named: imageFile) {
                ...
                self.imageView.frame = CGRect(x: 0, y: 0, width: 316, height: 209)
            }
        case .none:
            self.animatedView = nil
        }
...
class SuccessAnimatedView: AnimatableView {
    ...
    func setupLayers() {
        ...
        circleLayer.lineCap = CAShapeLayerLineCap.round
        circleLayer.lineWidth = 4
        circleLayer.actions = ["strokeStart": NSNull(),
                               "strokeEnd": NSNull(),
                               "transform": NSNull() ]
        self.layer.addSublayer(circleLayer)
    }
